...
- **APK Build System Fixed with Java 17** (Aug 12): Resolved critical build failures caused by Java toolchain conflicts between Capacitor plugins (requiring Java 21) and available runtime (Java 17). Root cause: Capacitor plugins override project Java settings, causing "Cannot find Java installation matching requirements" errors. Solution: Implemented global Java 17 toolchain enforcement across ALL modules using gradle.beforeProject hooks, disabled auto-detection, and added explicit JAVA_HOME configuration. Build system now uses: Gradle 8.9-bin + AGP 8.7.2 + Java 17 (forced) + Android SDK 35.
