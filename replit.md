...
- **APK Build Challenge** (Aug 10): Persistent Java version conflicts between Capacitor 7.x (Java 21) and GitHub Actions (Java 17). The `:capacitor-android` task continues to fail despite multiple isolation strategies. Implementing completely isolated Cordova build in /tmp directory to avoid any project dependencies that reference Capacitor modules.
