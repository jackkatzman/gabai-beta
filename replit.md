...
- **APK Build Solution** (Aug 10): Created separate Cordova-only workflow to completely bypass Capacitor Java 21 conflicts. The new `cordova-only-build.yml` workflow builds APK in isolated environment with no Capacitor dependencies, using pure Cordova + Android SDK 35. Implemented aggressive SDK version updates across all build files to resolve compile SDK warnings. Added fallback Gradle assembleDebug for reliability.
