...
- **APK Build Solution** (Aug 11): Created comprehensive APK build system with multiple approaches. The new `manual-gradle-build.yml` workflow completely bypasses both Cordova and Capacitor, using pure Android Studio Gradle build with SDK 35. This eliminates all bundleRelease conflicts. Fallback `cordova-only-build.yml` available as backup option.
