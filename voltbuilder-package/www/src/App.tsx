...
function AppContent() {
  ...
  React.useEffect(() => {
    ...
    // If we have a user and we're on the root or loading page, navigate to chat
    if (!isLoading && user && (location === '/' || location === '')) {
  ...
  return (
    <Switch>
      ...
      {/* Show loading while checking authentication - but NOT if we have a user */}
      {isLoading && !user && (
        <Route>
          {() => (
