...
echo "🚀 Building GabAi APK package v53 - Fixed microphone plugin"
echo "================================================"
...
# 4. ADD CORDOVA.JS SCRIPT TAG - CRITICAL FOR PLUGINS TO WORK
echo "🔧 Adding cordova.js script tag..."
sed -i 's|</head>|    <script src="cordova.js"></script>\n</head>|' www/index.html

# 5. Force hash routing for APK
...
# 6. Add comprehensive script loading fixes
echo "🔧 Adding script loading fixes..."
cat > script-fix.js << 'EOF'
<script>
// APK v53 - Complete loading fix
(function() {
...
# 7. Add COMPLETE bearer-only authentication patch with ALL API redirects
echo "🔧 Adding complete API redirect for all endpoints..."
cat > bearer-patch.js << 'EOF'
<script>
// v53 - Complete API redirect for ALL endpoints
(function() {
  ...
  console.log('✅ v53 Complete API redirect patch applied');
})();
</script>
EOF
...
# Create config.xml with CORRECT MEDIA PLUGIN for microphone
echo "📝 Creating config.xml for v53 with correct media plugin..."
cat > config.xml << 'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<widget id="ai.gabai.app" version="1.0.53" xmlns="http://www.w3.org/ns/widgets" xmlns:cdv="http://cordova.apache.org/ns/1.0">
    <name>GabAi</name>
    ...
    <!-- CORRECT PLUGINS FOR MICROPHONE AND CONTACTS -->
...
zip -r gabai-apk-v53-microphone-fix.zip www config.xml voltbuilder-package.json

echo "✅ Build complete!"
echo ""
echo "📱 APK v53 - Fixed Microphone Support:"
echo "  ✅ cordova-plugin-media for audio recording"
echo "  ✅ cordova-plugin-file for audio file handling"
echo "  ✅ cordova.plugins.diagnostic for permissions"
echo "  ✅ cordova.js script tag included"
echo "  ✅ ALL localhost API calls redirect to gabai.ai"
echo "  ✅ Bearer-only auth for all requests"
echo "  ✅ Contacts plugin included"
echo ""
echo "📦 Output file: gabai-apk-v53-microphone-fix.zip"
echo ""
echo "🚀 Next steps:"
echo "  1. Deploy your app to gabai.ai (CRITICAL!)"
echo "  2. Upload gabai-apk-v53-microphone-fix.zip to VoltBuilder"
echo "  3. Build APK with VoltBuilder"
echo "  4. Microphone should work after granting permissions!"
echo ""
echo "Version: 1.0.53"
