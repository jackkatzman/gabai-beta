...
function UserProfilesTable() {
  ...
  const { data: profiles, isLoading, error: profilesError } = useQuery<UserProfile[]>({
    queryKey: ["/api/analytics/user-profiles"],
    refetchInterval: 60000, // Refresh every minute
    retry: false,
  });

  // Add debugging when data changes
  React.useEffect(() => {
    if (profiles) {
      console.log('📊 User profiles loaded:', profiles.length, 'profiles');
  ...
  React.useEffect(() => {
    console.log('📊 Profiles analysis:', {
      ...
      filteredProfiles: filteredAndSortedProfiles.length,
      currentFilter: filterBy,
      currentSort: sortBy,
      firstFewUsers: filteredAndSortedProfiles.slice(0, 5).map((u: UserProfile) => ({ 
        name: u.name, 
        created: u.createdAt,
        messages: u.messageCount 
      }))
    });
  }, [profiles, filteredAndSortedProfiles, filterBy, sortBy]);
