...
echo "==================================="
echo "Building GabAi APK v44"
echo "Removed Statusbar Plugin (Causing Error)"
echo "==================================="
...
# Create config.xml without statusbar plugin
echo "📝 Creating config.xml for v44 (No statusbar plugin)..."
cat > config.xml << 'EOF'
<?xml version="1.0" encoding="UTF-8"?>
<widget id="ai.gabai.app" version="1.0.44" xmlns="http://www.w3.org/ns/widgets" xmlns:cdv="http://cordova.apache.org/ns/1.0">
    <name>GabAi</name>
    ...
    <!-- Core plugins without statusbar to avoid error -->
...
# Create package.json with pinned appcompat version
echo "📋 Creating package.json for VoltBuilder (no statusbar)..."
...
zip -r gabai-apk-v44-no-statusbar.zip www res config.xml voltbuilder-package.json

echo "✅ Build complete!"
echo ""
echo "📱 APK v44 Features:"
echo "  - Removed cordova-plugin-statusbar (was causing error)"
echo "  - Kept ChatGPT's color fixes (#FFFFFFFF format)"
echo "  - Included colors.xml resource file"
echo "  - Pinned appcompat to 1.6.1"
echo "  - Core plugins: device, camera, file"
echo ""
echo "📦 Output file: gabai-apk-v44-no-statusbar.zip"
echo ""
echo "🚀 Next steps:"
echo "  1. Upload gabai-apk-v44-no-statusbar.zip to VoltBuilder"
echo "  2. Build APK with VoltBuilder"
echo "  3. Should build without statusbar error!"
echo ""
echo "Version: 1.0.44"
