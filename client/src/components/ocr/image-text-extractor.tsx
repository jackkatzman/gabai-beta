...
export function ImageTextExtractor() {
  ...
  const handleFileSelect = async (file: File) => {
    // Check if file has a type property and it's an image
    ...
    try {
      ...
      // Use apiRequest for proper APK handling
      const apiBase = (window as any).IS_APK || (window as any).IS_VOLTBUILDER_APK ? 'https://gabai.ai' : '';
  ...
  const addTextToChat = async () => {
    ...
    try {
      ...
      if (hasContactInfo && selectedFile) {
        ...
        const formData = new FormData();
        // ChatGPT Fix: Send both 'file' and 'image' fields for compatibility
        formData.append('file', selectedFile, selectedFile.name || 'photo.jpg');
        formData.append('image', selectedFile, selectedFile.name || 'photo.jpg');
        formData.append('userId', user.id);
        
        // ChatGPT Fix: Use apiRequest for proper auth headers
        const response = await apiRequest('/api/ocr/business-card', 'POST', formData);
              ...
              <Button
                ...
                onClick={async (e) => {
                  ...
                  if (CordovaDirect.isAvailable()) {
                    ...
                    try {
                      // Use media-capture for camera
                      const blob = await CordovaDirect.capturePhoto();
