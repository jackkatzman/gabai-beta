...
export function setupAuth(app: Express) {
  ...
  app.use(session({
    ...
    cookie: {
      secure: isProduction || shouldUseSecureCookies, // Use secure in production or on HTTPS
      httpOnly: true,
      sameSite: isProduction ? 'lax' as const : 'none' as const, // 'lax' for production same-site, 'none' for development cross-origin
  ...
  if (process.env.GOOGLE_CLIENT_ID && process.env.GOOGLE_CLIENT_SECRET) {
    ...
    passport.use(new GoogleStrategy({
      ...
      callbackURL: 'https://gabai.ai/api/auth/google/callback' // Always use production domain
  ...
  app.get('/api/auth/google', (req, res, next) => {
    // Always use production domain for callback URL
    const callbackURL = 'https://gabai.ai/api/auth/google/callback';
    
    console.log('🔄 Google OAuth starting with callback:', callbackURL);
    
    // Store mobile redirect params in session for callback
    if (req.query.redirect_uri) {
      (req.session as any).mobileRedirectUri = req.query.redirect_uri;
      (req.session as any).mobileState = req.query.state;
      console.log('📱 Stored mobile redirect URI:', req.query.redirect_uri);
    }
  ...
  app.get('/api/auth/google/callback',
    ...
    (req, res) => {
      ...
      // Check for mobile deep link redirect first
      const mobileRedirectUri = (req.session as any)?.mobileRedirectUri;
      const mobileState = (req.session as any)?.mobileState;
      
      if (mobileRedirectUri && mobileRedirectUri.startsWith('gabai://')) {
        console.log('🔗 Mobile OAuth: Redirecting to deep link', mobileRedirectUri);
        // Clear stored redirect info
        delete (req.session as any).mobileRedirectUri;
        delete (req.session as any).mobileState;
        // Redirect to deep link with token
        return res.redirect(`${mobileRedirectUri}?token=${encodeURIComponent(token)}&state=success`);
      }
  ...
  // Removed duplicate /api/auth/user endpoint - handled in routes.ts with Bearer token support
